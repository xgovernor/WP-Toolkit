...
if ( ! class_exists( 'WPLTK_Server' ) ) {
	class WPLTK_Server {
		...
		public function get_server() {
			...
			if ( defined( 'WPLTK_SERVER_OVERRIDE' ) ) {
				return WPLTK_SERVER_OVERRIDE;
			}
		...
		public function auto_prepend_config(): string {
			...
			} elseif ( $this->isApacheSuPHP() ) {
				$return = 'apache-suphp'; // Apache + SuPHP
			} elseif ( $this->isApache() && ! $this->isApacheSuPHP() && ( $this->isCGI() || $this->isFastCGI() ) ) {
				$return = 'cgi'; // Apache + CGI/FastCGI
			} elseif ( $this->isLiteSpeed() ) {
				$return = 'litespeed';
			} elseif ( $this->isNGINX() ) {
				$return = 'nginx';
			} elseif ( $this->isIIS() ) {
			...
			update_option( 'wpltk_auto_prepend_config', $return, true );
			return $return;
		}
